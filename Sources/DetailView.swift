...
struct HostDetail: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 6) {
                Circle().fill(host.online ? Color.green : Color.orange).frame(width: 6, height: 6)
                Text(host.status)
                Spacer()
            ...
            if let s = host.sample {
                ...
                HStack(spacing: 8) {
                    summary(L("CPU · {0} 逻辑核", s.cores), value: number(s.cpu, suffix: "%"), percent: s.cpu, color: .accentColor)
                    summary(L("系统内存"), value: String(format: "%.1f / %.1f GiB", s.memoryUsed, s.memoryTotal), percent: s.memoryTotal > 0 ? s.memoryUsed / s.memoryTotal * 100 : 0, color: .secondary)
                }
                if !s.gpus.isEmpty {
                    ...
                    VStack(spacing: 0) {
                        ...
                        ForEach(s.gpus) { gpu in
                            HStack(spacing: 6) {
                                Text(gpu.index).foregroundStyle(Color.accentColor).frame(width: 28, alignment: .leading).help(gpu.name)
                                Text(number(gpu.utilization, suffix: "%"))
                                    .frame(maxWidth: .infinity, alignment: .trailing)
                                    .padding(.vertical, 4)
                                    .background(alignment: .leading) {
                                        GeometryReader { proxy in
                                            RoundedRectangle(cornerRadius: 3).fill(Color.accentColor.opacity(0.16))
